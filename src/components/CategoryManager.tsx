...
const getMacroCategory = (cat: string) => {
  ...
  if (c.includes("cocktail") || c.includes("drink") || c.includes("birr") || c.includes("analcolic") || c.includes("caffetteria") || c.includes("digestivi")) {
    return "Bar & Cafè";
  }
  return "Extra / Altro";
};
...
export default function CategoryManager() {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (d.exists() && d.data().config) {
          ...
          const defaults: Record<string, { isTop: boolean; isDropdown: boolean }> = {};
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await setDoc(doc(db, "settings", "categories"), { config: categoryConfig });
    } catch (err) {
  ...
  return (
    <div className="bg-white p-6 md:p-12 rounded-[2rem] sm:rounded-[3.5rem] shadow-2xl border border-brand-gold/20">
      <h2 className="font-logo text-3xl sm:text-5xl mb-8 text-brand-black">Gestione Categorie</h2>
      <div className="mb-8 p-4 bg-brand-gold/10 rounded-2xl border border-brand-gold/30 flex gap-4">
        ...
        <p className="text-sm">
          Scegli quali categorie mostrare come bottoni estesi in cima alla pagina e quali raggruppare nel menù a tendina "Altre categorie".
        </p>
      </div>
      ...
      <div className="flex flex-col gap-6 mb-12">
        {Object.entries(groupedCategories).map(([macro, cats]) => {
          ...
          return (
            <div key={macro} className="border border-brand-black/10 rounded-3xl overflow-hidden bg-brand-paper shadow-sm">
              <button 
              ...
              >
                <div className="flex items-center gap-3">
                  <h3 className="font-black text-xl text-brand-black tracking-wide">{macro}</h3>
                  <span className="bg-brand-black/5 text-brand-black/60 px-3 py-1 rounded-full text-xs font-bold">
                    {cats.length} voci
                  </span>
                </div>
                {expandedMacros[macro] ? <ChevronDown className="text-brand-black/40" /> : <ChevronRight className="text-brand-black/40" />}
              </button>
              
              {expandedMacros[macro] && (
                <div className="p-4 flex flex-col gap-3 border-t border-brand-black/5">
                  {cats.map(cat => {
                    const config = categoryConfig[cat] || { isTop: false, isDropdown: false };
                    return (
                      <div key={cat} className="flex flex-col md:flex-row items-start md:items-center justify-between bg-white p-4 rounded-2xl border border-brand-black/10 gap-4 shadow-sm">
                        <span className="font-bold text-base text-brand-black ml-2">{cat}</span>
                        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                          <label className="flex items-center gap-2 cursor-pointer bg-brand-paper p-3 rounded-xl border border-brand-black/10 flex-1 md:flex-none hover:border-brand-gold transition-colors">
                            ...
                            />
                            <span className="text-xs font-black uppercase tracking-wide">Mostra esteso</span>
                          </label>
                          <label className="flex items-center gap-2 cursor-pointer bg-brand-paper p-3 rounded-xl border border-brand-black/10 flex-1 md:flex-none hover:border-brand-gold transition-colors">
                            <input 
                              type="checkbox" 
                              ...
                              className="w-5 h-5 accent-brand-black"
                            />
                            <span className="text-xs font-black uppercase tracking-wide">Menù a tendina</span>
                          </label>
                        </div>
                      </div>
                    );
      ...
      >
        <Check size={16} /> {isSaving ? "Salvataggio..." : "Salva Configurazione Categorie"}
      </button>
    </div>
  );
}
