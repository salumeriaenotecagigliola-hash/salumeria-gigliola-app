  ...
  return text
    ...
    .filter(
      (s) =>
        s.length > 2 &&
        s.length < 40 &&
        !s.toLowerCase().includes("servito") &&
        !s.toLowerCase().includes("accompagnato") &&
        !s.toLowerCase().includes("scegli"),
    )
    .map((name) => {
...
export function useCustomerState(props: Props) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "settings", "menu"), async (snap) => {
      if (snap.exists() && snap.data().products) {
        ...
        const pinsePadellino = localMenu.filter(
          (p) => p.category.it === "Pinse" || p.category.it === "Padellino",
        );
        let merged = false;

        for (const localItem of pinsePadellino) {
          const existing = remoteMenu.find(
            (r) =>
              r.name.it === localItem.name.it &&
              r.category.it === localItem.category.it,
          );
          if (!existing) {
            remoteMenu.push(localItem);
            merged = true;
          } else if (
            existing.description?.it !== localItem.description.it ||
        ...
        if (merged) {
          ...
          const isAdminUser =
            isManager || (userEmail !== "" && adminEmails.includes(userEmail));
  ...
  const categoriesOriginal = React.useMemo(() => {
    ...
    products.forEach((p) => {
      if (!seen.has(p.category.it)) {
        ...
        cats.push({
          it: p.category.it,
          local: p.category[lang] || p.category.it,
        });
      }
    });
  ...
  }, [categoriesOriginal, macrosVersion]);

  const [takeawayHours, setTakeawayHours] = useState<any>(null);
  const [customerOrdersSettings, setCustomerOrdersSettings] = useState({
  ...
  useEffect(() => {
    if (
      ...
      (!activeMacroCategory || !availableMacros.includes(activeMacroCategory))
    ) {
  ...
  const activeCategoriesOriginal = React.useMemo(() => {
    return categoriesOriginal.filter(
      (c) => getMacroCategory(c.it) === activeMacroCategory,
    );
  }, [categoriesOriginal, activeMacroCategory, macrosVersion]);

  const allCategories = categoriesOriginal.map((c) => c.local);

  const [extrasData, setExtrasData] = useState<Extra[]>([]);
  const [featuredCrossSellProducts, setFeaturedCrossSellProducts] = useState<
    string[]
  >([]);
  ...
  useEffect(() => {
    const unsubExtras = onSnapshot(
      ...
      (snapshot) => {
        setExtrasData(
          snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }) as Extra),
        );
      },
      (err) => {
        console.error(err);
    ...
    const unsubCrossSell = onSnapshot(
      doc(db, "settings", "crossSelling"),
      (doc) => {
    ...
    const unsubMacros = onSnapshot(
      doc(db, "settings", "macros"),
      (doc) => {
        if (doc.exists() && doc.data().config) {
          localStorage.setItem(
            "puglia_category_macros",
            JSON.stringify(doc.data().config),
          );
    ...
    const unsubTableMappings = onSnapshot(
      doc(db, "settings", "tables"),
      (doc) => {
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (matchedCategory) {
              ...
              const matchedObj = categoriesOriginal.find(
                (c) => c.local === matchedCategory,
              );
              if (matchedObj) {
  ...
  const [isJoined, setIsJoined] = useState<boolean>(
    !!initialTable || !!editMode,
  );
  const [customerMode, setCustomerMode] = useState<
    "welcome" | "menuOnly" | "orderTable" | "takeaway" | "callWaiterForm"
  >(
    initialTable === "Asporto"
      ? "takeaway"
      : isManager || initialTable || editMode
        ? "orderTable"
        : "welcome",
  );
  ...
  useEffect(() => {
    if (customerMode === "takeaway") {
      ...
      const prepTimeMinutes = isManager
        ? 0
  ...
  const isTakeawayClosed = React.useMemo(() => {
    ...
    if (!isOpenAtTime && config.openAfternoon && config.closeAfternoon) {
      ...
      isOpenAtTime =
        evaluationTime >= openAftTime && evaluationTime <= closeAftTime;
    }
  ...
  const [customerEditingIndex, setCustomerEditingIndex] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (recoveredOrderId) {
      unsubscribe = onSnapshot(
        doc(db, "orders", recoveredOrderId),
        (snap: any) => {
          if (snap.exists()) {
        ...
        (snapshot) => {
          ...
          const filtered = snapshot.docs
            ...
            .filter((o) => {
              ...
              const targetOfThisOrder =
                tableMappings[o.tableNumber] || o.tableNumber;
              if (activeTable === "Asporto") {
                return (
                  targetOfThisOrder === "Asporto" &&
                  o.customerName === customerName.trim() &&
                  (o.takeawayCode === recoveryCode.trim().toUpperCase() ||
                    o.takeawayCode === localStorage.getItem("lastTakeawayCode"))
                );
              }
            ...
            .filter((o) => {
              ...
              if (["pending", "preparing", "served"].includes(o.status))
                return true;
              if (o.status === "paid") {
          ...
          if (activeOrders.length > 0) {
            const virtualActiveOrder = activeOrders.reduce(
              (acc, curr) => {
            ...
            virtualActiveOrder.status = activeOrders.some(
              (x) => x.status === "pending",
            )
              ? "pending"
              : activeOrders.some((x) => x.status === "preparing")
                ? "preparing"
                : "served";
  ...
  const [waiterCooldownRemaining, setWaiterCooldownRemaining] =
    useState<number>(0);
    ...
    const checkCooldown = () => {
      ...
      if (lastCallTime) {
        ...
        if (timeDiff < 10 * 60 * 1000) {
          setWaiterCooldownRemaining(
            Math.ceil((10 * 60 * 1000 - timeDiff) / 1000),
          );
        } else {
  ...
  const handleCallWaiter = async () => {
    ...
    if (waiterCooldownRemaining > 0) {
      setTableError(
        `Hai già chiamato il cameriere. Attendi ${Math.ceil(waiterCooldownRemaining / 60)} minuti.`,
      );
      return;
    }
    ...
    setTableError("");
    try {
      const lastCallTime = localStorage.getItem(
        `lastWaiterCall_${callWaiterTable}`,
      );
      if (lastCallTime) {
        const timeDiff = Date.now() - parseInt(lastCallTime, 10);
        if (timeDiff < 10 * 60 * 1000) {
          setTableError(
            "Hai già chiamato il cameriere di recente. Attendi 10 minuti.",
          );
      ...
      if (isRecent) {
        ...
        setCheckingTable(false);
        const recentDoc = firestoreCheck.docs.sort(
          (a, b) =>
        ...
        if (recentDoc && recentDoc.data().createdAt) {
          localStorage.setItem(
            `lastWaiterCall_${callWaiterTable}`,
            recentDoc.data().createdAt.toMillis().toString(),
          );
        }
        return;
      }
  ...
  const handleRecoverOrder = async () => {
    ...
    try {
      const { getDocs, query, collection, where } =
        await import("firebase/firestore");
      const codeToSearch = recoveryCode
        .trim()
        .toUpperCase()
        .replace("GIG-", "");
      const q = query(
      ...
      const orders = activeSnap.docs.map(
      ...
      if (order) {
        ...
        setCustomerMode(
          order.tableNumber === "Asporto" ? "takeaway" : "orderTable",
        );
        setIsJoined(true);
      } else {
  ...
  const [bowlSize, setBowlSize] = useState<string>(
  ...
  const [substitutions, setSubstitutions] = useState<Record<string, string>>(
    {},
  ); // baseIngredientName -> extraId
  const [inferredBaseIngredients, setInferredBaseIngredients] = useState<
    { name: string; category: string }[]
  >([]);
  ...
  const openProductModal = (product: Product) => {
    ...
    const productExtras = extrasData.filter(
      (e) =>
        (e.targets || []).includes(product.name.it) ||
        (e.targets || []).includes(product.category.it),
    );
    const hasExistingBase =
    ...
    const rawIngredients = hasExistingBase
      ? product.baseIngredients!
      : inferIngredients(product.description.it, extrasData);
  ...
  const editCustomerCartItem = (index: number) => {
    ...
    if (item.customizationOptions) {
      ...
      setBowlSize(
        item.customizationOptions.bowlSize || "Dimensione Unica - € 10,50",
      );
      ...
      const hasCustomizations =
        (item.customizationOptions.removedIngredients &&
          item.customizationOptions.removedIngredients.length > 0) ||
        (item.customizationOptions.selectedExtras &&
          item.customizationOptions.selectedExtras.length > 0) ||
        (item.customizationOptions.manualExtras &&
          item.customizationOptions.manualExtras.length > 0) ||
        (item.customizationOptions.manualSubstitution &&
          item.customizationOptions.manualSubstitution.length > 0);
  ...
  const confirmAddToCart = () => {
    ...
    if (selectedProduct.name.it.toLowerCase().includes("focaccia farcita")) {
      if (!bowlSalume && !bowlFormaggio && itemNote !== "vuota") {
        setBowlError(
          "Focaccia: Scegli un salume/formaggio oppure specifica 'Senza niente (vuota)'.",
        );
        return;
      }
      if (itemNote === "vuota") {
      ...
      finalNote = finalNote
        ? `Opzione: ${itemNote} | Note: ${finalNote}`
        : `Opzione: ${itemNote}`;
    ...
    if (selectedProduct.requiresWeight) {
      ...
      setBowlError("");
      const focacciaDetails =
        !bowlSalume && !bowlFormaggio
          ? "Focaccia vuota"
  ...
  const generateReceiptPDF = async (order: any) => {
    ...
    doc.save(
      `Ricevuta_Tavolo_${order.tableNumber}_${order.takeawayCode ? order.takeawayCode : order.id.slice(-4).toUpperCase()}.pdf`,
    );
  };
  ...
  const handleOrderInitiation = () => {
    ...
    const hasDrinks = cartCategoryNames.some(
      (cat) =>
    ...
    const hasFood = cartCategoryNames.some(
      (cat) =>
        !cat.toLowerCase().includes("acqu") &&
        !cat.toLowerCase().includes("bibite") &&
        !cat.toLowerCase().includes("bevande") &&
    ...
    if (!hasDrinks && hasFood) {
      ...
      const missingCategories = Array.from(
        new Set(products.map((p) => p.category.it)),
      ).filter((cat) => !cartCategoryNames.includes(cat));
      const otherProducts = products.filter(
        (p) =>
          missingCategories.includes(p.category.it) &&
          p.available &&
          p.price > 0 &&
          !cart.some((c) => c.productId === p.id),
      );
    ...
    if (suggestions.length < 2 && featuredCrossSellProducts.length > 0) {
      const featured = products.filter(
        (p) =>
          featuredCrossSellProducts.includes(p.name.it) &&
          p.available &&
  ...
  const submitOrder = async (additionalItem?: OrderItem) => {
    ...
    if (!isManager) {
      if (isTakeaway && customerOrdersSettings.allowTakeawayOrders === false) {
        setGlobalError(
          "Gli ordini da asporto sono temporaneamente disattivati.",
        );
        return;
      }
      if (!isTakeaway && customerOrdersSettings.allowTableOrders === false) {
        setGlobalError(
          "Gli ordini dal tavolo sono temporaneamente disattivati.",
        );
        return;
      }
    }
    ...
    if (isTakeawayClosed && !isManager && isTakeaway) {
      setGlobalError(
        "Siamo spiacenti, gli ordini da asporto non sono disponibili per l'orario richiesto. Verifica gli orari di apertura o scegli un altro orario.",
      );
      return;
    }
    ...
    try {
      ...
      if (editMode && orderId) {
        ...
        await updateDoc(doc(db, ordersPath, orderId), {
          ...
          notes:
            isTakeaway && !orderNotes.includes("PER ASPORTO")
        ...
        if (existingOrderDoc) {
          ...
          const cartWithAdditions = activeCart.map((item) => ({
            ...item,
            notes: item.notes
              ? `${item.notes} | [AGGIUNTA]${redirectionNote}`
              : `[AGGIUNTA]${redirectionNote}`,
          }));
          ...
          setSentOrderId(
            existingOrderDoc.data().takeawayCode || existingOrderDoc.id,
          );
        } else {
          // Create new order
          ...
          const baseNotes = isTakeaway
            ? `PER ASPORTO ALLE ORE ${takeawayTime}${orderNotes ? " | " + orderNotes : ""}`
            : orderNotes;
          const additionNote = isAddition && !isTakeaway ? " [AGGIUNTA]" : "";
          const finalNotes = baseNotes + redirectionNote + additionNote;
          generatedTakeawayCode = isTakeaway
          ...
          const docRef = await addDoc(collection(db, ordersPath), {
            ...
            notes: finalNotes,
            items: cartForNewOrder.map((item) => ({
              ...item,
              deliveredQuantity: 0,
            })),
            total: activeTotal,
        ...
        if (isTakeaway) {
          ...
          const code =
            generatedTakeawayCode || existingOrderDoc?.data()?.takeawayCode;
          if (code) {
  ...
  const btnClasses =
    "w-10 h-10 flex items-center justify-center bg-transparent border-2 border-brand-black text-brand-black rounded-full hover:bg-brand-black hover:text-brand-gold active:scale-90 transition-all focus:outline-none";

  const relevantExtras = selectedProduct
    ? extrasData.filter(
        (e) =>
          (e.targets || []).includes(selectedProduct.name.it) ||
          (e.targets || []).includes(selectedProduct.category.it),
      )
    : [];
  const canCustomize =
    inferredBaseIngredients.length > 0 || relevantExtras.length > 0;
  ...
  return {
    ...
    hasStoredTakeaway,
    setHasStoredTakeaway,
    activeCategoriesOriginal,
    allCategories,
    extrasData,
    setExtrasData,
    featuredCrossSellProducts,
    setFeaturedCrossSellProducts,
    categoryConfig,
    setCategoryConfig,
    tableMappings,
    setTableMappings,
    getActiveTableNumber,
    getActiveTableLabel,
    activeCategory,
    setActiveCategory,
    showAllergensFAQ,
    ...
    editingCartIndex,
    setEditingCartIndex,
    customerEditingIndex,
    setCustomerEditingIndex,
    editCustomerCartItem,
    cartSubItemName,
    setCartSubItemName,
    cartSubItemPrice,
    setCartSubItemPrice,
    handleTakeawayConfirm,
    callWaiterTable,
    setCallWaiterTable,
    callWaiterSent,
    setCallWaiterSent,
    showCallWaiterModal,
    setShowCallWaiterModal,
    isScrolled,
    setIsScrolled,
    handleCallWaiter,
    handleRecoverOrder,
